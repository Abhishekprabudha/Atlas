...
const NAV = [
  ...
  {id:'demo',label:'Atlas Demo Launcher',icon:'▶',badge:'5'},
  {id:'runbook',label:'Presenter Runbook',icon:'▤'},
  ...
  {id:'data',label:'Demo Data Explorer',icon:'⌘'}
];

const TITLES = {
  dashboard:['OPERATIONS CONTROL TOWER','Cargo Mission Control'],
  demo:['PROJECT ATLAS','Scenario Demonstration Launcher'],
  runbook:['PRESENTER MODE','Bid-Defense Demo Runbook'],
  ...
  data:['SYNTHETIC DEMO DATA','Operational Data Explorer']
};
...
class AtlasApp {
  constructor(){
    this.state = {
      ...
      selectedAwb:null, notifications:3, messageLog:[], schedules:[], injectedFault:false, demoAll:false,
      presenterIndex:0
    };
  }

  async init(){
    const [req, scenarios, seed] = await Promise.all([
      fetch('data/requirements.json').then(r=>r.json()),
      fetch('data/scenarios.json').then(r=>r.json()),
      fetch('data/seed-data.json').then(r=>r.json())
    ]);
    this.reqData=req; this.scenarios=scenarios; this.seed=seed;
  ...
  resetOperationalData(render=true){
    ...
    this.state.activeScenarioId=null; this.state.coverage=new Set(); this.state.messageLog=[]; this.state.injectedFault=false;
    this.data.tasks=[
  ...
  bindGlobal(){
    ...
    document.addEventListener('change', e=>{
      ...
      if(e.target.id==='demoSpeed'){this.state.speed=Number(e.target.value);}
      if(e.target.id==='dataTab'){this.state.dataTab=e.target.value;this.render();}
    });
    $('#launchScenarioBtn').addEventListener('click',()=>this.navigate('demo'));
    $('#presenterModeBtn').addEventListener('click',()=>this.openPresenter());
  ...
  render(){
    const map={dashboard:()=>this.dashboard(),demo:()=>this.demoLauncher(),runbook:()=>this.runbook(),outbound:()=>this.operations('S1'),inbound:()=>this.operations('S2'),wms:()=>this.wms(),reports:()=>this.reports(),maintenance:()=>this.maintenance(),integration:()=>this.integration(),requirements:()=>this.requirements(),data:()=>this.dataExplorer()};
  ...
  action(action, ds){
    const fn={
      ...
      pauseScenario:()=>this.pauseScenario(), nextStep:()=>this.nextStep(true), resetDemo:()=>this.resetDemo(),
      injectException:()=>this.injectException(ds.type||'destination'), openRequirement:()=>this.openRequirement(ds.id),
      ...
      replayMessage:()=>this.replayMessage(ds.id), simulateApiFailure:()=>this.simulateApiFailure(),
      ...
      setDataTab:()=>{this.state.dataTab=ds.tab;this.render();}, resetData:()=>{this.resetOperationalData();this.toast('Demo data reset','Synthetic terminal data returned to the prepared baseline.');},
      downloadJson:()=>this.downloadJson(), closeModal:()=>this.closeModal(), openPresenter:()=>this.openPresenter(), closePresenter:()=>this.closePresenter(),
      presenterNext:()=>this.presenterMove(1), presenterPrev:()=>this.presenterMove(-1), fullscreen:()=>document.documentElement.requestFullscreen?.(),
  ...
  currentScenario(){return this.scenarios.find(s=>s.id===this.state.activeScenarioId);}
  ...
  startScenario(id){
    ...
    this.toast(`Scenario ${this.currentScenario().number} started`,this.currentScenario().name);
    this.nextStep();
  }

  pauseScenario(){
    this.state.paused=!this.state.paused;this.state.running=!this.state.paused;
    this.toast(this.state.paused?'Scenario paused':'Scenario resumed',this.currentScenario()?.name||'');
  ...
  executeStep(sc,step){
    ...
    this.state.messageLog.unshift({id:uid('MSG'),time:this.state.simDate.toLocaleTimeString('en-GB',{hour12:false}),direction:step.system==='LMS'?'LMS → WMS':'WMS → LMS',event:step.event,status:'ACK',payload:{scenario:sc.id,step:step.id,awb:this.demoAwb(sc.id),location:step.location,correlationId:uid('CORR')}});
    ...
    if(sc.id==='S5'&&this.state.currentStep===4) this.simulateApiFailure(false);
  }

  demoAwb(id){return id==='S2'?'232-18945571':'232-48392175';}
  ...
  finishScenario(){
    ...
    this.toast(`Scenario ${sc.number} complete`,`${sc.name} demonstrated end-to-end.`);
    this.addEvent(`Scenario ${sc.number} completed`,`${sc.steps.length} steps · requirements evidence captured`,'success');
    if(this.state.demoAll){const idx=this.scenarios.findIndex(x=>x.id===sc.id);if(idx<this.scenarios.length-1)setTimeout(()=>this.startScenario(this.scenarios[idx+1].id),1000);else{this.state.demoAll=false;this.toast('All Atlas scenarios complete','Five scenarios demonstrated with dynamic evidence.');}}
    this.render();
  }

  runAllScenarios(){this.state.demoAll=true;this.startScenario('S1');}
  resetDemo(){this.state.running=false;this.state.paused=false;this.state.currentStep=-1;this.state.completedSteps=[];this.state.activeScenarioId=null;this.state.demoAll=false;this.render();this.toast('Scenario reset','Ready for presenter-controlled demonstration.');}

  injectException(type){
    ...
    return `
      ...
      <section class="layout-main" style="margin-top:16px">
        ...
        <div class="card span-all">
          <div class="card-header"><div><div class="card-title">Project Atlas Demonstration Readiness</div><div class="card-sub">Every scenario is interactive, traceable to the workbook and supported by dynamic synthetic data.</div></div><button class="primary-btn" data-route="demo">Open demo launcher</button></div>
          ${this.scenarioCards()}
        </div>
      </section>`;
  }
  ...
  terminalMap(){return `<div class="terminal-map">
    ...
    <div class="zone asrs">Zone 02<strong>Automated Storage</strong><small>ULD · IHCA · BCS · CSS</small><div style="margin-top:18px;display:grid;gap:12px"><span><i class="machine-dot"></i> Crane 01 · Running</span><span><i class="machine-dot ${this.state.injectedFault?'down':''}"></i> Crane 02 · ${this.state.injectedFault?'Fault':'Running'}</span></div></div>
  ...
  scenarioCards(selected=this.state.activeScenarioId){return `<div class="scenario-strip">${this.scenarios.map(s=>`<article class="scenario-card ${selected===s.id?'selected':''}" data-action="selectScenario" data-id="${s.id}"><div class="scenario-no">SCENARIO ${s.number}</div><div class="scenario-name">${s.icon} ${esc(s.short)}</div><div class="scenario-desc">${esc(s.name)}</div><div class="scenario-meta"><span class="badge">${s.steps.length} steps</span><span class="badge success">Ready</span></div></article>`).join('')}</div>`;}

  demoLauncher(){
    const sc=this.currentScenario()||this.scenarios[0]; const idx=this.state.currentStep;
    return `<div class="grid">
      <div class="card"><div class="card-header"><div><div class="card-title">Select the evaluation scenario</div><div class="card-sub">Run one scenario, advance step-by-step, or execute the complete Project Atlas walkthrough.</div></div><div class="card-actions"><button class="ghost-btn" data-action="resetDemo">Reset</button><button class="secondary-btn" data-action="runAll">Run all 5</button></div></div>${this.scenarioCards(sc.id)}</div>
      <div class="demo-runner">
        <div class="card flat"><div class="card-header"><div><div class="card-title">Scenario runbook</div><div class="card-sub">Presenter-controlled sequence</div></div></div><div class="runbook">${sc.steps.map((st,i)=>`<div class="run-step ${idx===i?'active':''} ${this.state.completedSteps.includes(st.id)?'complete':''}"><div class="step-index">${this.state.completedSteps.includes(st.id)?'✓':i+1}</div><div><div class="run-step-title">${esc(st.title)}</div><div class="run-step-sub">${esc(st.sub)}</div></div></div>`).join('')}</div></div>
        <div class="card demo-stage"><div class="card-header"><div><span class="badge info">Scenario ${sc.number}</span><h2 style="margin-top:10px">${esc(sc.name)}</h2><p>${esc(sc.purpose)}</p></div><span class="badge success">${this.state.running?'Running':idx>=sc.steps.length-1?'Complete':'Ready'}</span></div>
          ...
          ${idx>=0?this.stepEvidence(sc,sc.steps[idx]):`<div class="empty-state"><div class="empty-icon">▶</div><h3>Ready to demonstrate</h3><p>Start the scenario to generate live LMS commands, WMS tasks, WCS execution and audit evidence.</p></div>`}
          <div class="demo-controls"><button class="primary-btn" data-action="${this.state.running?'pauseScenario':'startScenario'}" data-id="${sc.id}">${this.state.running?'Ⅱ Pause':'▶ Start scenario'}</button><button class="secondary-btn" data-action="nextStep">Next step →</button><select class="control-select" id="demoSpeed"><option value="1" ${this.state.speed===1?'selected':''}>1× speed</option><option value="2" ${this.state.speed===2?'selected':''}>2× speed</option><option value="4" ${this.state.speed===4?'selected':''}>4× speed</option></select><button class="danger-btn" data-action="injectException" data-type="machine">⚠ Inject machine fault</button></div>
        </div>
      </div>
  ...
  operations(id){
    const sc=this.scenarios.find(s=>s.id===id); const relevant=this.data.shipments.filter(s=>s.direction===(id==='S1'?'Outbound':'Inbound'));
    ...
    return `<div class="grid grid-4">${this.kpi('Active shipments',relevant.length,'AWB-level visibility')}${this.kpi('Tasks open',this.data.kpis.tasksOpen,'Across WMS execution')}${this.kpi(id==='S1'?'Flight cutoff risk':'Customs / release holds',id==='S1'?'1 flight':'1 shipment','Live operational queue','warn')}${this.kpi('Inventory accuracy',`${fmt(this.data.kpis.inventoryAccuracy,1)}%`,'AWB · media · location')}</div>
      <div class="layout-main" style="margin-top:16px"><div class="card"><div class="card-header"><div><div class="card-title">${esc(sc.name)} live flow</div><div class="card-sub">Use the buttons below to run the exact Atlas scenario and watch cargo move across terminal zones.</div></div><button class="primary-btn" data-action="startScenario" data-id="${id}">▶ Run ${sc.short}</button></div>${this.terminalMap()}<div class="process-flow" style="margin-top:12px">${sc.steps.map((s,i)=>`<div class="process-node ${i===this.state.currentStep&&this.state.activeScenarioId===id?'active':''}">${esc(s.title)}<small>${esc(s.event)}</small></div>${i<sc.steps.length-1?'<span class="process-arrow">→</span>':''}`).join('')}</div></div>
      ...
      <div class="card span-all"><div class="card-header"><div><div class="card-title">Cargo shipment worklist</div><div class="card-sub">Dynamic AWB, ULD/media, flight, location and operational state</div></div><button class="ghost-btn" data-action="injectException" data-type="weight">Test weight mismatch</button></div>${this.shipmentTable(relevant)}</div></div>`;
  }
  ...
  triggerFault(id='TTV-01',render=true){const m=this.data.machines.find(x=>x.id===id);if(m){m.status='Fault';m.health=Math.max(55,m.health-20);m.availability=Math.max(97.6,m.availability-.8);this.state.injectedFault=true;this.data.kpis.openExceptions++;this.data.kpis.availability=Math.max(98.0,this.data.kpis.availability-.2);this.addEvent('Machine fault raised',`${m.name} · drive sensor anomaly · task rerouted`,'error');this.toast('Maintenance fault injected',`${m.name} is down. Alternate route activated.`,'error');if(render)this.render();}}
  recoverMachine(id,render=true){const m=this.data.machines.find(x=>x.id===id);if(m){m.status='Running';m.health=92;m.availability=98.8;this.state.injectedFault=false;this.data.kpis.openExceptions=Math.max(0,this.data.kpis.openExceptions-1);this.data.kpis.availability=98.6;this.addEvent('Machine returned to service',`${m.name} · repair and safety test complete`,'success');this.toast('Equipment recovered',`${m.name} returned to service.`);if(render)this.render();}}
  ...
  integration(){return `<div class="grid grid-4">${this.kpi('Interface availability','99.98%','LMS ↔ WMS gateway')}${this.kpi('Messages processed',fmt(this.data.kpis.commandsToday),'Today')}${this.kpi('Average response','214 ms','Target < 400 ms')}${this.kpi('Error queue',this.state.messageLog.filter(m=>m.status==='ERROR').length,'Replay enabled',this.state.messageLog.some(m=>m.status==='ERROR')?'warn':'')}</div>
      <div class="card" style="margin-top:16px"><div class="card-header"><div><div class="card-title">Executor integration model</div><div class="card-sub">LMS is the primary interface. WMS validates and decides. WCS / PLC executes physical movement.</div></div><button class="danger-btn" data-action="simulateApiFailure">Simulate interface failure</button></div><div class="message-grid"><div class="system-node"><div class="system-icon">L</div><h3>LMS / CMS</h3><p>Shipment master · commands · milestones</p></div><div class="flow-arrow">⇄</div><div class="system-node"><div class="system-icon">W</div><h3>RoboOps WMS</h3><p>Validation · tasks · locations · exceptions</p></div><div class="flow-arrow">⇄</div><div class="system-node"><div class="system-icon">⚙</div><h3>WCS / PLC</h3><p>ASRS · conveyors · TTV · UWS</p></div></div></div>
  ...
  requirements(){const total=this.reqData.requirements.length,cov=[...this.state.coverage].length;const categories=['All',...new Set(this.reqData.requirements.map(r=>r.category))];const sheets=Object.keys(this.reqData.rawWorkbook);return `<div class="grid grid-4">${this.kpi('Workbook scenarios','5 / 5','Outbound · inbound · reports · maintenance · integration')}${this.kpi('Requirement groups',total,'Every description available')}${this.kpi('Demo evidence',`${cov}/${total}`,'Captured during simulations')}${this.kpi('Coverage design','100%','Mapped to application modules')}</div>
      <div class="grid grid-3" style="margin-top:16px"><div class="card span-2"><div class="card-header"><div><div class="card-title">Requirement-to-demonstration matrix</div><div class="card-sub">Search the entire workbook, open exact descriptions and jump to the relevant demonstration.</div></div><span class="badge success">Source: Project Atlas XLSX</span></div><div class="toolbar"><input id="requirementSearch" class="text-input" placeholder="Search function, clause or keyword…" value="${esc(this.state.requirementSearch)}"><select class="control-select" id="requirementCategory">${categories.map(c=>`<option ${c===this.state.requirementCategory?'selected':''}>${c}</option>`).join('')}</select></div><div id="requirementList" class="requirements-list"></div></div>
      <div class="card"><div class="card-header"><div><div class="card-title">Coverage summary</div><div class="card-sub">Requirements are linked to scenarios, pages and live evidence.</div></div></div>${this.scenarios.map(s=>{const reqs=this.reqData.requirements.filter(r=>r.scenarioIds.includes(s.id));const done=reqs.filter(r=>this.state.coverage.has(r.id)).length;return `<div style="margin-bottom:15px"><div class="metric-row"><span>S${s.number} · ${s.short}</span><strong>${done}/${reqs.length}</strong></div><div class="progress-bar"><i style="width:${reqs.length?Math.max(5,done/reqs.length*100):0}%"></i></div></div>`}).join('')}<button class="primary-btn full" data-action="runAll">Generate all demo evidence</button></div></div>
  ...
  openRequirement(id){const r=this.reqData.requirements.find(x=>x.id===id);if(!r)return;this.openModal(`<div class="modal-header"><div><span class="badge info">${r.id} · ${r.category}</span><h2 style="margin-top:10px">${esc(r.name)}</h2></div><button class="modal-close" data-action="closeModal">×</button></div><p class="card-sub">Source sheet: ${esc(r.source)} · Scenario: ${r.scenarioIds.join(', ')}</p><pre>${esc(r.description)}</pre><div class="demo-controls"><button class="primary-btn" data-action="jumpRequirement" data-id="${r.id}">Open demonstration</button></div>`);}
  jumpRequirement(id){const r=this.reqData.requirements.find(x=>x.id===id);if(!r)return;this.closeModal();this.selectScenario(r.scenarioIds[0]);this.navigate('demo');this.toast('Requirement loaded',`${id} · ${r.name}`);}

  runbook(){return `<div class="grid grid-3"><div class="card span-2"><div class="card-header"><div><div class="card-title">90-minute bid-defense demo choreography</div><div class="card-sub">A presenter-safe sequence that links each screen to the Atlas evaluation focus.</div></div><button class="primary-btn" data-action="openPresenter">Open full-screen presenter</button></div><div class="timeline">${[['Opening thesis',0,8],['S1 Outbound',8,24],['S2 Inbound',30,20],['S3 Reporting',52,11],['S4 Maintenance',65,11],['S5 Integration',77,9],['Close & Q&A',88,9]].map((x,i)=>`<div class="timeline-row"><span>${x[0]}</span><div class="timeline-track"><i class="timeline-block" style="left:${x[1]}%;width:${x[2]}%;background:${i===5?'linear-gradient(90deg,var(--orange),var(--amber))':''}"></i></div><strong>${i===0?'5 min':i===6?'Q&A':`${8+i*2} min`}</strong></div>`).join('')}</div></div><div class="card"><div class="card-title">Three proof moments</div><div style="display:grid;gap:11px;margin-top:13px"><div class="machine-card"><strong>01 · Command execution</strong><div class="card-sub">LMS command produces WMS task, physical move and status return.</div></div><div class="machine-card"><strong>02 · Operational visibility</strong><div class="card-sub">Dashboards expose throughput, inventory, health and exceptions.</div></div><div class="machine-card"><strong>03 · Governed readiness</strong><div class="card-sub">Integration, cyber, DR, availability and support are credible.</div></div></div></div></div>
      <div class="card" style="margin-top:16px"><div class="card-header"><div><div class="card-title">Presenter cues by scenario</div><div class="card-sub">Use these as speaker notes during the live product demonstration.</div></div></div><div class="grid grid-3">${this.scenarios.map(s=>`<article class="machine-card"><span class="badge info">Scenario ${s.number}</span><h3>${s.name}</h3><p class="card-sub"><strong>Say:</strong> ${esc(this.presenterCue(s.id))}</p><p class="card-sub"><strong>Show:</strong> ${s.steps.slice(0,3).map(x=>x.title).join(' → ')}…</p><button class="secondary-btn" data-action="startScenario" data-id="${s.id}">Run scenario</button></article>`).join('')}</div></div>`;}
  presenterCue(id){return ({S1:'“The LMS remains the cargo master; RoboOps WMS executes every terminal movement and returns actual status.”',S2:'“Inbound routing is rule-aware: PRP remains stored, TTS moves directly to cold room, and release is authorized by LMS.”',S3:'“Management receives accurate, filterable and scheduled evidence — not static screenshots.”',S4:'“Availability is measurable and defendable: every downtime minute is linked to a fault, work order and recovery.”',S5:'“The executor model is integration-safe: idempotent commands, monitored acknowledgements, retry and replay.”'})[id];}

  dataExplorer(){const tabs=['shipments','media','machines','locations','events','tasks'];const rows=this.data[this.state.dataTab]||[];return `<div class="card"><div class="card-header"><div><div class="card-title">Synthetic operational data</div><div class="card-sub">Prepared for live demonstration. Every scenario changes this data in real time.</div></div><div class="card-actions"><button class="ghost-btn" data-action="downloadJson">Download JSON</button><button class="danger-btn" data-action="resetData">Reset data</button></div></div><div class="toolbar">${tabs.map(t=>`<button class="filter-chip ${this.state.dataTab===t?'active':''}" data-action="setDataTab" data-tab="${t}">${t}</button>`).join('')}</div>${Array.isArray(rows)&&rows.length?this.genericTable(rows):'<div class="empty-state">No data for this tab.</div>'}</div>`;}

  showAwb(id){const s=this.data.shipments.find(x=>x.awb===id);if(!s)return;const events=this.data.events.filter(e=>e.detail.includes(id)||e.detail.includes(s.media));this.openModal(`<div class="modal-header"><div><span class="badge ${s.direction==='Outbound'?'success':'info'}">${s.direction}</span><h2 style="margin-top:10px">AWB ${s.awb}</h2></div><button class="modal-close" data-action="closeModal">×</button></div><div class="grid grid-3" style="margin-top:15px">${[['Flight',s.flight],['Route',s.route],['Special handling',s.type],['Pieces',s.pieces],['Weight',`${fmt(s.weight)} kg`],['Media',s.media],['Location',s.location],['Status',s.status],['Cutoff',s.cutoff]].map(x=>`<div class="machine-card"><div class="card-sub">${x[0]}</div><strong>${x[1]}</strong></div>`).join('')}</div><h3>Related events</h3>${events.length?this.eventList.call({data:{events}},10):'<div class="card-sub">Current event chain will populate as the scenario runs.</div>'}`);}

  openPresenter(){this.state.presenterIndex=0;const div=document.createElement('div');div.id='presenterOverlay';div.className='presenter-overlay';$('#modalLayer').append(div);this.renderPresenter();}
  renderPresenter(){const root=$('#presenterOverlay');if(!root)return;const items=[{title:'MAB Kargo Warehouse Modernisation',body:'A live proof that LMS remains the orchestrator, RoboOps WMS becomes the terminal execution layer, and every physical move is traceable.',cue:'Open with the operating thesis. Do not start with generic product features.'},...this.scenarios.map(s=>({title:`Scenario ${s.number} · ${s.name}`,body:s.purpose,cue:this.presenterCue(s.id)})),{title:'Closing position',body:'One integrated cargo operating layer: process-aligned, API-ready, availability-led and built to unlock future automation.',cue:'Close on confidence: all five scenarios, all IT topics, one coherent operating model.'}];const x=items[this.state.presenterIndex];root.innerHTML=`<div class="presenter-main"><div class="eyebrow">AIonOS × MAB KARGO · PROJECT ATLAS</div><h1>${esc(x.title)}</h1><p>${esc(x.body)}</p><div class="demo-controls"><button class="ghost-btn" data-action="presenterPrev">← Previous</button><button class="primary-btn" data-action="presenterNext">Next →</button><button class="ghost-btn" data-action="fullscreen">Full screen</button><button class="danger-btn" data-action="closePresenter">Exit</button></div></div><aside class="presenter-panel"><div class="card-title">Presenter cue</div><div class="presenter-cue" style="margin-top:14px">${esc(x.cue)}</div><div class="presenter-kpis"><div class="presenter-kpi"><strong>${this.state.presenterIndex+1}/${items.length}</strong><span>STORY POSITION</span></div><div class="presenter-kpi"><strong>${fmt(this.data.kpis.availability,1)}%</strong><span>LIVE AVAILABILITY</span></div><div class="presenter-kpi"><strong>5/5</strong><span>ATLAS SCENARIOS</span></div><div class="presenter-kpi"><strong>6/6</strong><span>IT TOPICS</span></div></div><div class="card-sub" style="margin-top:auto">Keyboard-style controls are represented by the Previous / Next buttons. Exit returns to the application.</div></aside>`;}
  presenterMove(d){const max=this.scenarios.length+1;this.state.presenterIndex=Math.max(0,Math.min(max,this.state.presenterIndex+d));this.renderPresenter();}
  closePresenter(){document.exitFullscreen?.();$('#presenterOverlay')?.remove();}
  ...
  downloadJson(){this.downloadBlob(JSON.stringify(this.data,null,2),'mab-kargo-demo-data.json','application/json');}
...
new AtlasApp().init().catch(err=>{
  console.error(err);document.querySelector('#view').innerHTML=`<div class="card"><h2>Unable to load demo data</h2><p>${esc(err.message)}</p><p>Serve this repository through GitHub Pages or a local HTTP server; browsers block JSON fetches when index.html is opened directly as a file.</p></div>`;
});
