...
class AtlasApp {
  ...
  operations(id){
    ...
    return `<div class="grid grid-4">${this.kpi('Active shipments',relevant.length,'AWB-level visibility')}${this.kpi('Tasks open',this.data.kpis.tasksOpen,'Across WMS execution')}${this.kpi(id==='S1'?'Flight cutoff risk':'Customs / release holds',id==='S1'?'1 flight':'1 shipment','Live operational queue','warn')}${this.kpi('Inventory accuracy',`${fmt(this.data.kpis.inventoryAccuracy,1)}%`,'AWB · media · location')}</div>
      ...
      <div class="card span-all"><div class="card-header"><div><div class="card-title">ULD shipment trace</div><div class="card-sub">Trace each ULD label to every shipment label currently consolidated within it.</div></div><span class="badge info">${id==='S1'?'Outbound':'Inbound'} manifest</span></div>${this.uldTraceTable(relevant)}</div>
  ...
  uldTraceTable(shipments){
    const traces=[...new Map(shipments.filter(s=>this.data.media.some(m=>m.id===s.media&&m.type==='ULD')).map(s=>[s.media,this.data.media.find(m=>m.id===s.media)])).entries()];
    return `<div class="table-wrap"><table class="data-table uld-trace-table"><thead><tr><th>ULD label</th><th>Shipment labels (AWB)</th><th>Current location</th><th>ULD state</th><th></th></tr></thead><tbody>${traces.map(([uld,media])=>{const linked=shipments.filter(s=>s.media===uld);return `<tr><td><strong class="mono">${esc(uld)}</strong><br><small>${esc(media.type)} · ${fmt(media.capacity)}% utilized</small></td><td><div class="trace-labels">${linked.map(s=>`<button class="trace-label" data-action="showAwb" data-id="${esc(s.awb)}">${esc(s.awb)}</button>`).join('')}</div></td><td>${esc(media.location)}</td><td><span class="badge ${media.status==='In-move'?'info':'success'}">${esc(media.status)}</span></td><td><button class="ghost-btn" data-action="showUldTrace" data-id="${esc(uld)}">View trace</button></td></tr>`;}).join('')}</tbody></table></div>`;
  }
  ...
  showAwb(id){const s=this.data.shipments.find(x=>x.awb===id);if(!s)return;const events=this.data.events.filter(e=>e.detail.includes(id)||e.detail.includes(s.media));this.openModal(`<div class="modal-header"><div><span class="badge ${s.direction==='Outbound'?'success':'info'}">${s.direction}</span><h2 style="margin-top:10px">AWB ${esc(s.awb)}</h2></div><button class="modal-close" data-action="closeModal">×</button></div><div class="grid grid-3" style="margin-top:15px">${[['Flight',s.flight],['Route',s.route],['Special handling',s.type],['Pieces',s.pieces],['Weight',`${fmt(s.weight)} kg`],['ULD label',s.media],['Location',s.location],['Status',s.status],['Cutoff',s.cutoff]].map(x=>`<div class="machine-card"><div class="card-sub">${x[0]}</div><strong>${esc(x[1])}</strong></div>`).join('')}</div><div class="trace-callout"><span class="card-sub">ULD-to-shipment trace</span><strong class="mono">${esc(s.media)} → ${esc(s.awb)}</strong><button class="ghost-btn" data-action="showUldTrace" data-id="${esc(s.media)}">View full ULD manifest</button></div><h3>Related events</h3>${events.length?this.eventList.call({data:{events}},10):'<div class="card-sub">Current event chain will populate as the workflow runs.</div>'}`);}

  showUldTrace(id){const media=this.data.media.find(m=>m.id===id);const shipments=this.data.shipments.filter(s=>s.media===id);if(!media)return;this.openModal(`<div class="modal-header"><div><span class="badge ${shipments[0]?.direction==='Outbound'?'success':'info'}">${esc(shipments[0]?.direction||'ULD')} ULD trace</span><h2 style="margin-top:10px">ULD ${esc(media.id)}</h2><p class="card-sub">${esc(media.location)} · ${esc(media.status)} · ${fmt(media.capacity)}% utilized</p></div><button class="modal-close" data-action="closeModal">×</button></div><div class="trace-flow"><div class="trace-uld"><span class="card-sub">ULD label</span><strong class="mono">${esc(media.id)}</strong></div><span class="trace-arrow">→</span><div><span class="card-sub">Shipment labels</span><div class="trace-labels">${shipments.map(s=>`<button class="trace-label" data-action="showAwb" data-id="${esc(s.awb)}">${esc(s.awb)}</button>`).join('')}</div></div></div>${this.shipmentTable(shipments)}`);}
